...
\begin{remark} \label{rem:Remark2} \normalfont{\textbf{(interpolation results.)} Let us consider an interval $D = \{l, r\} \in \R$ where $f$ and $f^\epl$ are defined and $f^\epl$ to be a polynomial interpolation of $f$ on a grid $x_i = l + \epl i, i = 0, \dots, N, r = l + N\epl$. Then, the interpolation error of can be estimated if $f$ is regular enough. In particular, we can state the following results  
\begin{enumerate}
	\item function $f$ Lipschitz continuous of constant $K$, $f^\epl$ piecewise constant interpolation computed at the midpoint of each subinterval of the grid
		\begin{equation*}
		...
		\end{equation*}
	\item function $f$ of class $\mathcal{C}^1$, $f^\epl$ piecewise constant interpolation, $C_1$ a real constant \cite[Chapter 8]{Quarteroni2007}
		\begin{equation*}
			\|f - f^\epl\|_{\infty} \leq C_1 \epl \|f'(x)\|_{\infty}.
		\end{equation*}
	\item function $f$ of class $\mathcal{C}^2$, $f^\epl$ piecewise linear interpolation, $C_2$ a real constant \cite[Chapter 8]{Quarteroni2007}
		\begin{equation*}
